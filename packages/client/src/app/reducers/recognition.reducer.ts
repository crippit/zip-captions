...
export const recognitionReducers = createReducer(defaultRecognitionState,
  ...
  on(RecognitionActions.saveZoomToken, (state: RecognitionState, action: { apiToken: string}) => ({...state, zoomApiToken: action.apiToken })),
  on(RecognitionActions.clearZoomToken, (state: RecognitionState) => ({...state, zoomApiToken: undefined}))
)